import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router";
import PageMeta from "../../../components/common/PageMeta";
import PageBreadcrumb from "../../../components/common/PageBreadCrumb";
import ComponentCard from "../../../components/common/ComponentCard";
import { useState } from "react";
import Button from "../../../components/ui/button/Button";
import { PencilIcon, PlusIcon } from "../../../icons";
import { QueryPagination } from "../../../components/pagination/QueryPagination";
import { Divider } from "../../../components/dividers/Divider";
import { ResponsiveTable } from "../../../components/tables/ResponsiveTable";
import { usePosIntegrationsQuery } from "../../../hooks/queries/implementations/usePosIntegrationsQuery";
import { PosIntegration } from "../../../hooks/api/Dtos/posIntegrations/PosIntegration";
import { useIntegrationHelper } from "../../../utilities/useIntegrationHelper";

export const PosIntegrationsPage = () => {
    const { t } = useTranslation();
    const navigate = useNavigate();
    const integrationHelper = useIntegrationHelper();

    const [state, setState] = useState({
        page: 0,
        pageSize: 25,
        deleteEntity: undefined as PosIntegration | undefined,
    })
    const integrationsQuery = usePosIntegrationsQuery({
        page: state.page,
        pageSize: state.pageSize,
    })

    return <>
        <PageMeta
            title={t("pages.posIntegrations.title")}
            description={t("quivi.product.description")}
        />
        <PageBreadcrumb pageTitle={t("pages.posIntegrations.title")} />

        <ComponentCard title={t("pages.posIntegrations.title")}>
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
                <div className="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
                    <Button
                        size="md"
                        variant="primary"
                        startIcon={<PlusIcon />}
                        onClick={() => navigate("add")}
                    >
                        {
                            t("common.operations.new", {
                                name: t("common.entities.posIntegration")
                            })
                        }
                    </Button>
                </div>
                <div className="max-w-full overflow-x-auto">
                    <ResponsiveTable
                        isLoading={integrationsQuery.isFirstLoading}
                        name={{
                            key: "name",
                            label: t("common.name"),
                            render: d => integrationHelper.getStrategyName(d.type)
                        }}
                        actions={[
                            {
                                render: () => <PencilIcon className="size-5" />,
                                key: "edit",
                                label: t("common.edit"),
                                onClick: d => navigate(`/admin/integrations/${d.id}/edit`),
                            },
                            // {
                            //     render: () => <TrashBinIcon className="size-5" />,
                            //     key: "delete",
                            //     label: t("common.delete"),
                            //     onClick: d => setState(s => ({ ...s, deleteEntity: d}))
                            // },
                        ]}
                        data={integrationsQuery.data}
                        getKey={d => d.id}
                    />
                    <Divider />
                    <QueryPagination
                        query={integrationsQuery}
                        onPageIndexChange={p => setState(s => ({ ...s, page: p }))}
                        pageSize={state.pageSize}
                    />
                </div>
            </div>
        </ComponentCard>
    </>
}